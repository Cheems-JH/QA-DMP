###########################################################
# Learning 2D multi-quality demos (X,Y) with DMP and QA-DMP
###########################################################
import numpy as np
...
all_demos = np.load('~/dataset/2D_simulation_demos.npy', allow_pickle=True).tolist()
baseline_x = []
baseline_y = []
y_demos = []
x_demos = []
...
#     if i == 0:
#         plt.plot(x_demos[i], y_demos[i], 'b--', label='Demos')
#     else:
#         plt.plot(x_demos[i], y_demos[i], 'b--', label=None)
# plt.plot(baseline_x, baseline_y, 'r--', label='Desired demo')
# plt.tight_layout()
# plt.show()
...
if os.path.exists(x_ts2vec_model_path):
    ...
    x_space2_ts2vec_model.fit(x_space2_ts2vec_dataset, n_epochs=100, n_iters=100, verbose=True)
...
if os.path.exists(y_ts2vec_model_path):
    y_space2_ts2vec_model.load(y_ts2vec_model_path)
    print("Saved Y TS2Vec model loaded.")
else:
    print("Training Y TS2Vec model...")
    y_space2_ts2vec_model.fit(y_space2_ts2vec_dataset, n_epochs=100, n_iters=100, verbose=True)
...
# Visualization: The result of assigning weights to each data point in each dimension (the deeper the color, the better the relative quality is considered)
# fig, axes = plt.subplots(2, 1, figsize=(8, 9))
# ax = axes[0]
# all_y_flat = np.array(x_demos_space2).flatten()             # (N_demos * L_demos,)
# all_t_flat = np.tile(bm_s_x, N_demos)  # (N_demos * L_demos,)
# all_weights_flat = x_space2_final_weights      # (N_demos * L_demos,)
# sort_idx = np.argsort(all_weights_flat)
# all_y_sorted = all_y_flat[sort_idx]
# all_t_sorted = all_t_flat[sort_idx]
# all_weights_sorted = all_weights_flat[sort_idx]
# cmap = matplotlib.colormaps['Blues']
# scatter1 = ax.scatter(all_t_sorted, all_y_sorted, c=all_weights_sorted, cmap=cmap, s=5)
# ax.plot(bm_s_x, baseline_f_target_x, 'r-', label='x baseline target forcing term')
# ax.legend(frameon=False, loc='best')
# ax = axes[1]
# all_y_flat = np.array(y_demos_space2).flatten()             # (N_demos * L_demos,)
# all_t_flat = np.tile(bm_s_y, N_demos)  # (N_demos * L_demos,)
# all_weights_flat = y_space2_final_weights      # (N_demos * L_demos,)
# sort_idx = np.argsort(all_weights_flat)
# all_y_sorted = all_y_flat[sort_idx]
# all_t_sorted = all_t_flat[sort_idx]
# all_weights_sorted = all_weights_flat[sort_idx]
# cmap = matplotlib.colormaps['Blues']
# scatter2 = ax.scatter(all_t_sorted, all_y_sorted, c=all_weights_sorted, cmap=cmap, s=5)
# ax.plot(bm_s_y, baseline_f_target_y, 'r-', label='y baseline target forcing term')
# ax.legend(frameon=False, loc='best')
# plt.tight_layout()
# plt.show()
...
for i in range(cnt_sample):
    if i == 0:
        plt.plot(x_space4_outputs[i], y_space4_outputs[i], 'g-', label='Outputs')
    else:
        plt.plot(x_space4_outputs[i], y_space4_outputs[i], 'g-')
plt.plot(baseline_x, baseline_y, 'r--', label='Desired demo')
plt.xlabel('X')
plt.ylabel('Y')
...
for i in range(cnt_sample):
    if i == 0:
        plt.plot(x_space2_outputs[i], y_space2_outputs[i], 'g-', label='Outputs')
    else:
        plt.plot(x_space2_outputs[i], y_space2_outputs[i], 'g-')
plt.plot(baseline_x, baseline_y, 'r--', label='Desired demo')
plt.xlabel('X')
plt.ylabel('Y')
plt.legend(frameon=False, loc='best')
...
def compute_compactness(x_list, y_list):
    N = len(x_list)
    pairwise_dists = []

    for i, j in combinations(range(N), 2):
        xi, yi = x_list[i], y_list[i]
        xj, yj = x_list[j], y_list[j]
        dist = np.mean(np.sqrt((xi - xj) ** 2 + (yi - yj) ** 2))
        pairwise_dists.append(dist)

    return np.mean(pairwise_dists)

def compute_average_jerk(x_list, y_list):
    jerks = []
...
def compute_average_distance_to_baseline(x_list, y_list, baseline_x, baseline_y):
    dists = []
    for x, y in zip(x_list, y_list):
        dist = np.sqrt((x - baseline_x)**2 + (y - baseline_y)**2)
        dists.append(np.mean(dist))
    return np.mean(dists)

compactness_space2 = compute_compactness(x_space2_outputs, y_space2_outputs)
jerk_space2 = compute_average_jerk(x_space2_outputs, y_space2_outputs)
dist_to_baseline_space2 = compute_average_distance_to_baseline(x_space2_outputs, y_space2_outputs, baseline_x, baseline_y)

compactness_space4 = compute_compactness(x_space4_outputs, y_space4_outputs)
jerk_space4 = compute_average_jerk(x_space4_outputs, y_space4_outputs)
dist_to_baseline_space4 = compute_average_distance_to_baseline(x_space4_outputs, y_space4_outputs, baseline_x, baseline_y)
